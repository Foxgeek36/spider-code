...
class ShopsBaseinfo(RedisSpider):
    ...
    def __init__(self, *a, **kw):
        super(ShopsBaseinfo, self).__init__(*a, **kw)
        con = get_mysql_client()
        cursor = con.cursor(cursor = pymysql.cursors.DictCursor)
        cursor.execute(self.sql)
        datas = cursor.fetchall()
        for data in datas:
            url = self.api.format(data['regionid'],data['cateid'],data['cityid'])
            self.redis_cli.sadd('shops_baseinfo:start_urls',url)
        self.logger.info(f'{len(datas)}条 列表api初始连接成功插入redis')
